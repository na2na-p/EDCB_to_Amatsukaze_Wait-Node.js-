...
db.all('select * from rec_history where is_encoded == 0', (err, rows) => {
	for (let index = 0; index < rows.length; index++) {
		...
		const send = (execSync('"' + config[0].Addtask_path + '" -r "' + config[0].Amatsukaze_root + '" -f "' + rows[index].recorded_path + '" -ip "' + config[0].serverip + '" -p ' + config[0].port + ' -o "' + config[0].saveFolder + '" -s "' + encode_preset.name + '" --priority 3 --no-move'));
		console.log('ok');
	}
});
